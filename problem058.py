...
But for computer it is not handy. Computers want anything to be numbers. So instead it is nice to
represent a card with a single integer value and derive its suit and rank when neccessary.
...
Now choose some arrangement of suits and of ranks (usually, relevant to your game) and put them into array
of strings:
...
You will be given several cards represented by their values (from 0 to 51). You will need to print out
their names.
...
def card_names():
    naipes = ['Clubs', 'Spades', 'Diamonds', 'Hearts']
    ranks = ['2', '3', '4', '5', '6', '7', '8', '9', '10', 'Jack', 'Queen', 'King', 'Ace']
    dados = [12, 38, 7, 5, 9, 8, 35, 26, 14, 34, 4, 31, 10, 48, 24, 22, 15, 37, 50, 47, 11, 43]
    carta, nome_cartas, cartas = [], '', []
    for k in range(0, len(dados)):
        naipe = round(dados[k], 0) / 13
        naipe = int(naipe)
        carta.append(naipes[naipe])
        valor = dados[k] % 13
        carta.append(ranks[valor])
    for k in range(0, len(carta), 2):
        nome_cartas = carta[k + 1] + '-' + 'of' + '-' + carta[k]
        cartas.append(nome_cartas)
    return f'{[k for k in cartas]}'.replace('[', '').replace(']', '').replace(',', '').replace("'", '')
