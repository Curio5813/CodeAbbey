...
Paul read an article on Standard Deviation and has the new idea now. He at once realized that the main problem is
that the broker (the company who provides service for playing at market) takes comission of 1% for each deal. So
regarding example above Paul loses 40 dounds when buying shares and 46 more when selling them back. So the real
profit is not 600 but only 600 - 40 - 46 = 514 dounds.

It is obvious for him now, that he should prefer operations with shares which are more volatile - i.e. the price
of which changes in wider range, so that his profit from changing price is more significant when compared to
broker's comission.

For example, if initial price of shares was 50 (rather than 20) and it had grown to 52 when Paul sold them, his
profit for 200 shares would be only 400 dounds. However, comission would be 100 dounds when buying and 104 when
selling, so his real gain is only 196 dounds - more than half money was taken by broker!
...
comission (per share) is 1 dound. Standard deviation would be 1 dound also, so these shares do not deserve being
bought or sold.
...
def share_price_volatile():
    from statistics import stdev, mean
    texto, dados, n, share, dado, prices, choice = '', [], 0, [], [], [], []
    with open('share_price_volatile.txt') as arquivo:
        lista = list(arquivo)
