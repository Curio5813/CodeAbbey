...
For any type of card game it is important to be able to shuffle the deck. Since only few of the programming
languages have built-in functions to randomly shuffle an array (like PHP) it is necessary to learn some useful
algorithm.
...
so that CQ is the "Queen of Clubs", HT is the "Ten of Hearts", D2 is the "Two of Diamonds" and SA is the "Ace of
Spades".

There are 52 cards in total, so they should be put into an array of 52 elements before shuffling. Initial
ordering of the array is the following: 13 cards of Clubs, then 13 cards of Diamonds, then 13 cards of Hearts
and at last 13 cards of Spades. Inside of each suit cards are arranged from Ace to King, so the whole deck looks
like:
...
to position 15, where D3 was - and D3 should be moved back to position 0. Then we take C2 from the position 1 and
...
them to required range by taking modulo 52, like in the Double Dice Roll task. Perform the shuffling and print
the new order of cards as an answer.
...
def cards_shuffling():
    with open('cards_shuffling01.txt') as arquivo:
        cards = [k for k in arquivo.readline().split(' ')]
    with open('cards_shuffling02.txt') as arquivo:
